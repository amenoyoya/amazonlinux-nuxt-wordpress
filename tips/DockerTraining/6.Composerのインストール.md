...
- PHPコンテナに入ってコマンドを実行しても良いが、Docker環境ならDockerfileに記述した方が良い（`6-php_composer-apache`フォルダを参考に）
  - 以下の、`# -- Composer用設定 --` ～ `# -- /End Composer Setings --` までがComposerインストール用の設定
    ```ruby
    # php/Dockerfile
    FROM php:7-apache
...
### Monologの動作確認
- 動作確認用PHP作成（`share\training\html\index.php`）
  ```php
  <?php // index.php
  ...
  ```
- `http://192.168.56.101`にアクセス
  - `share\training\html\log\error.log`が作成され、指定したログが記述されていればOK
